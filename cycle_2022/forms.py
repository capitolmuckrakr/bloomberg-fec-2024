...
class SummaryForm(forms.Form):
    ...
    min_date = forms.CharField(label="Min filing date (YYYYMMDD)", required=False)
    max_date = forms.DateField(label="Max filing date (YYYYMMDD)", required=False)
    bloomberg_committees = forms.BooleanField(label=mark_safe('Restrict to Saved Candidates (<a href="/admin/cycle_2022/candidate/">Add/Edit candidates list</a>)'), required=False) #This really belongs in the template, but WTH
