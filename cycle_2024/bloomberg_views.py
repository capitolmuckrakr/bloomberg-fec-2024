...
def summary(request):
    ...
    csv_url = reverse('2024:summary_csv') + "?"+ request.GET.urlencode()
    cycle_summary_url = reverse('2024:cycle_summary') + "?"+ request.GET.urlencode().replace('period_','cycle_')
    
    paginator = Paginator(results, 50)
    page = request.GET.get('page')
    results = paginator.get_page(page)
    return render(request, '2024/summary_001.html', {'form': form, 'results':results, 'opts': ScheduleA._meta,'csv_url':csv_url, 'cycle_summary_url':cycle_summary_url, 'contact':settings.CONTACT})
...
def cycle_summary(request):
    ...
    csv_url = reverse('2024:cycle_summary_csv') + "?"+ request.GET.urlencode()
    period_summary_url = reverse('2024:summary') + "?"+ request.GET.urlencode().replace('cycle_','period_')
    
    paginator = Paginator(results, 50)
    page = request.GET.get('page')
    results = paginator.get_page(page)
    return render(request, '2024/cycle_summary.html', {'form': form, 'results':results, 'opts': ScheduleA._meta, 'csv_url':csv_url, 'period_summary_url':period_summary_url, 'contact':settings.CONTACT})
