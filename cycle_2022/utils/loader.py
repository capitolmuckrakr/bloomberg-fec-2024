...
from cycle_2022.utils import logging
from cycle_2022.utils.unreadable_files import readable_file_check, delete_file
...
logger = canon_logging.getLogger('bloomberg-fec.'+__name__)
logger.setLevel(LOGLEVEL)

ACCEPTABLE_FORMS = ['F3','F3X','F3P','F24', 'F5']
BAD_COMMITTEES = ['C00401224','C00630012','C00694323'] #actblue; it starts today; winred
...
from cycle_2022.cycle_settings import CYCLE
...
def check_coverage_dates(filing, coverage_end, myextra=None):
    ...
    if coverage_end:
        ...
        try:            
            ...
            if myextra:
                myextra['TAGS']="bloomberg-fec, result:fail"
                logger.error("Invalid format for filing {}, creating filing and marking as FAILED".format(filing),extra=myextra)
...
def download_filings(filings, filing_dir="filings/", myextra=None):
    ...
    try:
        if filings:
            user_agent = { 'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:103.0) Gecko/20100101 Firefox/103.0', 'From': settings.CONTACT }
                ...
                if filename not in existing_filings:
                    ...
                    #    sys.stdout.write("we already have filing {} downloaded\n".format(filing))
                        ...
                        try:
                            ...
                            logger.error("Failed to download filing {} from FEC site: {}".format(filing,e),extra=myextra)
        else:
            logger.warning('Missing required list of filings to download or list is empty',extra=myextra)
    except Exception as e:
        logger.error("Failed to download from FEC site: {}".format(e),extra=myextra)
    ...
    if debug:
        ...
        for line in skeds:
            ...
            if line['form_type'].startswith('SE') or line['form_type'].startswith('F57'):
                ...
                if office == "P":
                    line['bloomberg_district'] = "PRESIDENT"
                elif office_exists and state_exists and district_exists:
                    line['bloomberg_district'] = "{}-{}".format(state,district)
                elif office == 'S':
                    if state_exists:
                        line['bloomberg_district'] = "{}-SEN".format(state)
                    else:
                        line['bloomberg_district'] = "UNKNOWN"
                elif state_exists:
                    if state in ['AK','DE','MT','ND','SD','VT','WY']:
                        line['bloomberg_district'] = "{}-AT-LARGE".format(state)
                    else:
                        line['bloomberg_district'] = "{}-HOUSE-UNKNOWN".format(state)
                else:
                    line['bloomberg_district'] = "UNKNOWN"
...
def reassign_standardized_donors(filing_id, amended_id, myextra=None):
    ...
    for transaction in matched_transactions:
        ...
        if len(new_trans) == 0:
            if myextra:
                myextra['TAGS']="bloomberg-fec, result:warning"
...
def is_even_year(filing,myextra=None):
    try:
        ...
        try:
            ...
            logger.error('Could not find coverage date, not fixing sums', extra=myextra)
            return
    if year % 2 == 0:
        return True
    return False
...
def evaluate_filing_file(filename, filing_id, myextra=None):
    if myextra:
        myextra=myextra.copy()
        myextra['FILING']=str(filing_id)
    logger.debug("Evaluating {} for loading into database".format(filing_id),extra=myextra)
    form_line =''
...
#                form_line = next(reader)
        ...
        fec_file_delimiter = '\x1c'
        fixed_form_line = []
        for line in form_line:
            if fec_file_delimiter in line:
                for item in line.split(fec_file_delimiter):
                    fixed_form_line.append(item)
                form_line = fixed_form_line
...
def load_filing(filing, filename, filing_fieldnames, myextra=None):
    ...
    if myextra:
        myextra=myextra.copy()
        myextra['FILING']=str(filing)
        _ = myextra.pop('TRANSACTION',None)
    ...
    if myextra:
        myextra['TAGS']="bloomberg-fec, result:success"
...
def load_filings(filing_dir, myextra=None):
    ...
    for filename in filing_csvs:
        ...
        try:
            ...
            if myextra:
                myextra['TAGS']="bloomberg-fec, result:warn"
