...
from cycle_2022.models import *
from cycle_2022.forms import *
...
def index(request):
    return render(request, '2022/index.html', { 'contact':settings.CONTACT})
...
def filings(request):
    ...
    return render(request, '2022/filings.html', {'form': form, 'results':results, 'opts': ScheduleA._meta, 'contact':settings.CONTACT})
...
def contributions(request):
    ...
    if not request.GET:
        return render(request, '2022/contributions.html', {'form': form,  'contact':settings.CONTACT})
    ...
    csv_url = reverse('2022:contributions_csv') + "?"+ request.GET.urlencode()
    ...
    return render(request, '2022/contributions.html', {'form': form, 'results':results, 'results_sum':results_sum, 'csv_url':csv_url, 'contact':settings.CONTACT})
...
def expenditures(request):
    ...
    if not request.GET:
        return render(request, '2022/expenditures.html', {'form': form, 'contact':settings.CONTACT})
    ...
    csv_url = reverse('2022:expenditures_csv') + "?"+ request.GET.urlencode()

    paginator = Paginator(results, 50)
    page = request.GET.get('page')
    results = paginator.get_page(page)

    return render(request, '2022/expenditures.html', {'form': form, 'results':results, 'results_sum':results_sum, 'csv_url':csv_url, 'contact':settings.CONTACT})
...
def get_ie_results(request):
    ...
    bloomberg_district = request.GET.get('bloomberg_district')
    min_date = request.GET.get('min_date')
    max_date = request.GET.get('max_date')
    ...
    if bloomberg_district:
        results = results.filter(bloomberg_district=bloomberg_district)
    if min_date:
        results = results.filter(expenditure_date__gte=min_date)
    if max_date:
        results = results.filter(expenditure_date__lte=max_date)
...
def ies(request):
    ...
    if not request.GET:
        return render(request, '2022/ies.html', {'form': form, 'contact':settings.CONTACT})

    results = get_ie_results(request)

    results_sum = results.aggregate(Sum('expenditure_amount'))

    csv_url = reverse('2022:ie_csv') + "?"+ request.GET.urlencode()

    paginator = Paginator(results, 50)
    page = request.GET.get('page')
    results = paginator.get_page(page)
    context = {'form': form, 'results':results, 'results_sum':results_sum, 'csv_url':csv_url, 'contact':settings.CONTACT}
    if request.GET.get('bloomberg_district'):
        context['bloomberg_district'] = True
        context['opts'] = ScheduleE._meta

    return render(request, '2022/ies.html', context)
...
def races(request):
    races = ScheduleE.objects.filter(active=True).values('bloomberg_district').annotate(Sum('expenditure_amount'))
    
    order_by = request.GET.get('order_by', 'expenditure_amount')
    if order_by == 'race':
        races = races.order_by('bloomberg_district')
    else:
        races = races.order_by('-expenditure_amount__sum')

    return render(request, '2022/races.html', {'races':races, 'contact':settings.CONTACT})

def top_donors(request):
    donors = sorted(Donor.objects.all(), key=lambda d: d.contribution_total_2022, reverse=True)
    ...
    results = paginator.get_page(page)
    opts = Donor._meta
    return render(request, '2022/top_donors.html', {'results':results, 'contact':settings.CONTACT, 'opts':opts})
...
def filing_status(request, status):
    ...
    page = request.GET.get('page')
    results = paginator.get_page(page)
    return render(request, '2022/filing_status.html', {'results':results, 'contact':settings.CONTACT})
...
def committee(request, committee_id):
    ...
    return render(request, '2022/committee.html', context)
...
def candidates(request):
    ...
    context['csv_url'] = reverse('2022:candidates_csv') + "?"+ request.GET.urlencode()
    return render(request, '2022/candidates.html', context)
...
def inaugural(request):
    ...
    if not request.GET:
        return render(request, '2022/inaugural.html', {'form': form})
    ...
    page = request.GET.get('page')
    results = paginator.get_page(page)
    return render(request, '2022/inaugural.html', {'form':form, 'results':results})
